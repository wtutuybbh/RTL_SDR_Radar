...
//экранные в полярные
PolarCoord ScreenConversions::screenToPolar(const QPointF &screenCenter ,const QPointF &xy)
{
...
PolarCoord ScreenConversions::decartToPolar(const QPointF &p, double toNorth)
{
    ...
    return PolarCoord(rtAngl,hypot(p.x(), p.y()));
}

//полярные в декартовые
QPointF ScreenConversions::polarToDecart(const PolarCoord &plr)
{
...
//полярные в экранные
QPointF ScreenConversions::polarToScreen(const QPointF &screenCenter ,const PolarCoord &plr)
{
...
{
    return polarToScreen(screenCenter, PolarCoord(angl,r));
}
